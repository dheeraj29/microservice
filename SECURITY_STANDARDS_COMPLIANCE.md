...
* **Model Context Protocol (MCP) Governance**: Spring AI 2.0.0 Modern Streamable HTTP `@McpTool` endpoints enforce method-level `@PreAuthorize("hasAnyRole('USER', 'ADMIN')")` for read operations and `@PreAuthorize("hasRole('ADMIN')")` for mutations. Plain header spoofing is rejected; calls require valid cryptographic Bearer JWTs or session cookies.
* **Actuator & Swagger Port Isolation**: All administrative Actuator and Swagger UI endpoints are physically bound to isolated internal management ports (`9081`, `9083`, `9084`). The public application ports (`8081`, `8083`, `8084`) and API Gateway (`8080`) return `404 Not Found` / `401 Unauthorized` for all observational endpoints.
...
| :--- | :--- | :--- |
| **Unauthorized Endpoint Access** | Role-Based Access Control (RBAC) enforced via `@PreAuthorize("hasRole('ADMIN')")` and `@PreAuthorize("hasAnyRole('USER', 'ADMIN')")` across all REST controllers and MCP tools. | [`AdminController.java`](file:///c:/Personal-Project/microservice-main/microservice-main/adminservice/src/main/java/com/da/demo/adminservice/controller/AdminController.java), [`BookingController.java`](file:///c:/Personal-Project/microservice-main/microservice-main/bookingservice/src/main/java/com/da/demo/bookingservice/controller/BookingController.java), [`InventoryController.java`](file:///c:/Personal-Project/microservice-main/microservice-main/inventoryservice/src/main/java/com/da/demo/inventoryservice/controller/InventoryController.java) |
| **Actuator & Swagger Exposure** | Actuator and Swagger UI are completely relocated to internal management ports (`9081`, `9083`, `9084`) with `springdoc.use-management-port=true`. Public edge returns 404 / 401. | [`application.properties`](file:///c:/Personal-Project/microservice-main/microservice-main/adminservice/src/main/resources/application.properties) (`management.server.port=9081`) |
...
| **Long-Lived Leaked Access Tokens / JWT Staleness Window** | **Periodic Bounded-Staleness Validation (30s Window)**: Embedded in `SessionRecord` via `lastValidatedAt`. Employs **Distributed Double-Checked Locking (DCL)** (`lock:validate:<sid>`) in Valkey: a single thread introspects Keycloak (`/protocol/openid-connect/token/introspect`) and auto-heals via refresh token or instantly purges revoked sessions. Waiting threads re-read Valkey to prevent thundering herds while guaranteeing zero false positives. | [`SessionRecord.java`](file:///c:/Personal-Project/microservice-main/microservice-main/common-security/src/main/java/com/da/demo/security/model/SessionRecord.java), [`DistributedSessionManager.java`](file:///c:/Personal-Project/microservice-main/microservice-main/common-security/src/main/java/com/da/demo/security/session/DistributedSessionManager.java), [`BffSessionAuthenticationFilter.java`](file:///c:/Personal-Project/microservice-main/microservice-main/common-security/src/main/java/com/da/demo/security/filter/BffSessionAuthenticationFilter.java) |
| **Direct Bearer Token Forgery / Expiration Bypass** | Direct `Authorization: Bearer <jwt>` requests (M2M / Swagger / AI Agents) are verified cryptographically using cached **Keycloak JWKS RSA-256** signatures (`/protocol/openid-connect/certs`) and strict expiration (`exp > now`) and issuer (`iss`) checks. | [`KeycloakAuthService.java`](file:///c:/Personal-Project/microservice-main/microservice-main/common-security/src/main/java/com/da/demo/security/service/KeycloakAuthService.java), [`BffSessionAuthenticationFilter.java`](file:///c:/Personal-Project/microservice-main/microservice-main/common-security/src/main/java/com/da/demo/security/filter/BffSessionAuthenticationFilter.java) |
| **Native AI Tooling & Dynamic Client Registration (RFC 7591 & RFC 8252)** | AI/IDE tools (VS Code Copilot, Claude Desktop) connect via standard RFC 7591 dynamic registration. Guarded by 5 Keycloak policies: anonymous rate-limiting (`max-clients: 200`), scope sandboxing (`email`, `profile`, `roles`), protocol mapper restrictions, disabled full scope inheritance, and mandatory PKCE (`S256`). | [`keycloak/realm-export.json`](file:///c:/Personal-Project/microservice-main/microservice-main/keycloak/realm-export.json), [`GatewayMcpAggregatorController.java`](file:///c:/Personal-Project/microservice-main/microservice-main/gateway/src/main/java/com/da/demo/gateway/mcp/GatewayMcpAggregatorController.java) |
| **CSRF in Login Flow & Infinite Loop Traps** | Ephemeral `state` UUID tokens stored in Valkey (`pkce:state:<state>`) validate that the login callback originates from the exact client instance that started it. Route candidates are sanitized against `/login`, `/logout`, `/callback`, and `/` to eliminate redirect loops. | [`BffAuthController.java`](file:///c:/Personal-Project/microservice-main/microservice-main/common-security/src/main/java/com/da/demo/security/controller/BffAuthController.java), [`loginhandler.service.ts`](file:///c:/Personal-Project/microservice-main/microservice-main/angularplay/src/services/loginhandler.service.ts) |
